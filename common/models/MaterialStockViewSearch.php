 ...
 */
class MaterialStockViewSearch extends StockView {
    ...
    public function rules() {
        return [
            [['id', 'available_qty', 'status', 'CB', 'UB'], 'integer'],
            [['DOC', 'DOU', 'material_category', 'material_id'], 'safe'],
        ];
    }
    ...
    public function search($params) {
        ...
        if ($this->material_id != '') {
            $arr_data = [];
            $materials = SupplierwiseRowMaterial::find()->where(['like', 'item_code', $this->material_id])->orWhere(['like', 'item_name', $this->material_id])->orWhere(['like', 'reference', $this->material_id])->all();
            foreach ($materials as $material) {
                $arr_data[] = $material->id;
            }
            $query->andWhere(['material_id' => $arr_data]);
